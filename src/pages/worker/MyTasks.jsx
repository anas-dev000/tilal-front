...
import { Eye, Clock, MapPin, AlertCircle, CheckCircle, Calendar, Layers } from 'lucide-react';
...
const MyTasks = () => {
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    if (filter === 'active') return ['assigned', 'in-progress'].includes(task.status);
          ...
          >
            ...
            <div className="space-y-4 pl-2">
              ...
              <div className="flex justify-between items-start gap-2">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-semibold border flex items-center gap-1 flex-shrink-0 ${getStatusColor(task.status)}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {task.section && (
                      <div className="flex items-center gap-1 text-xs text-gray-600 mt-0.5">
                        ...
                        <span className="truncate">Section: {task.section}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {task.adminReview?.status === 'rejected' && task.adminReview?.comments && (
              ...
              {task.adminReview?.comments && task.adminReview?.status === 'pending' && (
              ...
              <Button
                className="w-full"
                variant={
                  task.status === 'assigned' ? 'primary' :
                  task.status === 'in-progress' ? 'success' :
                  'outline'
                }
                icon={Eye}
                onClick={(e) => {
              ...
              >
                ...
                {!['assigned', 'in-progress', 'completed'].includes(task.status) && 'View Details'}
              </Button>
